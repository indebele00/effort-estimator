...
function product(values: number[]): number { return values.reduce((a,v)=>a*v,1); }
function ceil(num: number, precision = 0): number { const p = Math.pow(10, precision); return Math.ceil(num * p) / p; }
function addWorkdays(start: Date, nDays: number): Date {
  let daysRemaining = nDays; const d = new Date(start); d.setHours(0,0,0,0);
  while (daysRemaining > 0) { d.setDate(d.getDate()+1); const day = d.getDay(); if (day !== 0 && day !== 6) daysRemaining -= 1; }
  return d;
}
function formatDateISO(d: Date | null): string {
  if (!d) return ""; const y = d.getFullYear(); const m = String(d.getMonth()+1).padStart(2,"0"); const day = String(d.getDate()).padStart(2,"0"); return `${y}-${m}-${day}`;
}
...
export default function FactorEffortEstimator() {
  ...
  function resetAll() {
    setRole('BA/PMO'); setBaseEffort(8); setFocusHoursPerDay(5); setAvailabilityFactor("100%");
    setTaskType("Feature"); setComplexity("Medium"); setCodeImpact("Modify"); setDependencies("Few");
    setTechNovelty("Mixed"); setMeetingsLoad("Medium"); setDeveloperLevel("Mid"); setLocBucket("100–300");
    setRisk("Medium"); setStartDateStr("");
  }
        ...
        <Card className="md:col-span-2 shadow-lg">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              <Button onClick={resetAll} className="rounded-2xl"><RefreshCw className="mr-2 h-4 w-4"/>Reset</Button>
            </div>
          </CardContent>
        </Card>
...
function KV({ label, value, emphasize }: { label: string; value: string | number; emphasize?: boolean }) {
  return (
    <div className={cn("flex flex-col p-3 border rounded-xl", emphasize && "bg-slate-50 font-semibold shadow-inner")}>
      <div className="text-slate-500">{label}</div>
      <div className="text-slate-900 text-base">{(value ?? "—").toString()}</div>
    </div>
  );
}
...
function NumberField({
  ...
  label: string; value: number; setValue: (v: number)=>void; min?: number; max?: number; step?: number; suffix?: string;
}) {
  return (
    <div>
...
function SelectField<T extends string>({
  ...
  label: string; value: T; setValue: (v: T)=>void; options: string[];
}) {
  return (
    <div>
      <Label className="text-sm">{label}</Label>
